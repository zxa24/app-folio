...
const els = {
  ...
  // ── A3a' project_meta panel ──
  projectMetaTabBtn: document.getElementById("projectMetaTabBtn"),
  projectMetaTab: document.getElementById("projectMetaTab"),
  projectMetaStatus: document.getElementById("projectMetaStatus"),
  projectMetaFields: document.getElementById("projectMetaFields"),
  pmProjectName: document.getElementById("pmProjectName"),
  pmSourceLang: document.getElementById("pmSourceLang"),
  pmTargetLang: document.getElementById("pmTargetLang"),
  pmWorkflowMode: document.getElementById("pmWorkflowMode"),
  pmDeadline: document.getElementById("pmDeadline"),
  pmTranslator: document.getElementById("pmTranslator"),
  pmTranslatorRemember: document.getElementById("pmTranslatorRemember"),
  pmDesigner: document.getElementById("pmDesigner"),
  pmProjectId: document.getElementById("pmProjectId")
};
...
// Canonical supported languages for the project_meta source/target dropdowns.
// MUST stay consistent with the InDesign toolkit's lib/lang_script_table.js
// LANG_SCRIPT_TABLE (the source of truth for the project's real source/target
// langs: en/fr/es/de + zh-CN/zh-TW/zh-HK/ja/ko + th/ar). Codes are BCP-47; the
// dropdown shows "Name (code)" to mirror the InDesign font panel's display. If
// the InDesign table gains a language, add it here too.
...
const state = {
  ...
  // ── A3a' project_meta ──
  // Working _meta object (schema-shaped). Populated on package load: read the
  // package _meta.json when present, else ProjectMeta.inferDefaults(). Edited
  // via the Project panel. Read-preserved + serialized into the package ZIP on
  // export (designer + other fields preserved — never build fresh).
  projectMeta: null,
  projectMetaConfidence: null,   // inferDefaults confidence map (null when from a real _meta.json)
  projectMetaFromPackage: false  // true when read from a real _meta.json
};
...
function init() {
  ...
  if (els.projectMetaTabBtn) {
    els.projectMetaTabBtn.addEventListener("click", () => setActiveTab("project_meta"));
  }
  wireProjectMetaPanel();
...
function scheduleUrlAutomation() {
  ...
  async function runAutomation() {
    try {
      ...
      // {silent:true}: this is the headless URL-param driver (e2e / Playwright /
      // cron) — it MUST take the modal-free load path. Without silent, the
      // non-silent branch pops the metadata modal and the unattended pipeline
      // deadlocks on a confirm no human is there to click. (Same reason the
      // onSaveAll alert is suppressed below.) impl-audit A-class.
      var loadResult = await loadFolderFromServer(pkg, { silent: true });
...
function initProjectMeta() {
  // Inject the canonical schema into the _meta core lib so validate()/serialize()
  // work. Guarded: if the lib chain failed to load, project_meta features degrade
  // gracefully (callers check `typeof ProjectMeta` before use).
  try {
    if (typeof ProjectMeta !== "undefined" && typeof ProjectMetaSchema !== "undefined") {
      ProjectMeta.setSchema(ProjectMetaSchema);
    }
  } catch (e) {
    console.warn("[project_meta] schema injection failed:", e);
  }
}
...
function loadThemeMode() {
  let raw = "";
  // Route through LocalPrefs (adopts the same THEME_STORAGE_KEY); fall back to
  // raw localStorage if the module failed to load.
  if (typeof LocalPrefs !== "undefined") {
    raw = LocalPrefs.get("theme_mode") || "";
  } else {
    try { raw = localStorage.getItem(THEME_STORAGE_KEY) || ""; } catch (e) {}
  }
  return safeThemeMode(raw);
}

function saveThemeMode(mode) {
  const m = safeThemeMode(mode);
  if (typeof LocalPrefs !== "undefined") {
    LocalPrefs.set("theme_mode", m);
  } else {
    try { localStorage.setItem(THEME_STORAGE_KEY, m); } catch (e) {}
  }
}
...
const TABS = {
  segments:     { btn: "segmentsTabBtn",   content: "segmentsTab" },
  ai:           { btn: "aiTabBtn",          content: "aiTab" },
  project_meta: { btn: "projectMetaTabBtn", content: "projectMetaTab" }
};

function setActiveTab(tab) {
  if (!Object.prototype.hasOwnProperty.call(TABS, tab)) tab = "segments";
  state.activeTab = tab;
  Object.keys(TABS).forEach(function (key) {
    const isActive = key === tab;
    const btn = els[TABS[key].btn];
    const content = els[TABS[key].content];
    if (btn) btn.classList.toggle("active", isActive);
    if (content) content.classList.toggle("active", isActive);
  });
  if (tab === "project_meta") renderProjectMetaPanel();
}

// ── A3a' project_meta panel ────────────────────────────────────────────────
// Working _meta lives in state.projectMeta (schema-shaped). The panel renders
// from it and edits write back. On package load (Task A3a'-4) the package
// _meta.json seeds it (read-preserve: designer + any other-role fields kept);
// absent → ProjectMeta.inferDefaults. On export it's serialized into the ZIP.

function pmEmptyMeta() {
  return {
    schema_version: (typeof ProjectMeta !== "undefined" ? ProjectMeta.CURRENT_SCHEMA_VERSION : 1),
    contributors: {}
  };
}

// Seed from inference (no real _meta.json in the package).
function setProjectMetaFromInference(ctx) {
  if (typeof ProjectMeta === "undefined") {
    state.projectMeta = pmEmptyMeta();
    state.projectMetaConfidence = null;
    state.projectMetaFromPackage = false;
  } else {
    const inf = ProjectMeta.inferDefaults(ctx || {});
    state.projectMeta = inf.data || pmEmptyMeta();
    if (!state.projectMeta.contributors) state.projectMeta.contributors = {};
    state.projectMetaConfidence = inf.confidence || {};
    state.projectMetaFromPackage = false;
  }
  autofillTranslator();
}

// Seed from a real _meta.json read out of the package (read-preserve: we keep
// every field the package recorded, including contributors.designer).
function setProjectMetaFromPackage(metaObj) {
  state.projectMeta = metaObj || pmEmptyMeta();
  if (!state.projectMeta.contributors) state.projectMeta.contributors = {};
  state.projectMetaConfidence = null;
  state.projectMetaFromPackage = true;
  autofillTranslator();
}

// Auto-fill translator from the remembered operator_name ONLY when the package
// hasn't recorded one — a package/user-recorded translator always wins.
function autofillTranslator() {
  if (!state.projectMeta) return;
  if (!state.projectMeta.contributors) state.projectMeta.contributors = {};
  const existing = state.projectMeta.contributors.translator;
  if (existing && String(existing).trim()) return;
  if (typeof LocalPrefs !== "undefined") {
    const remembered = LocalPrefs.get("operator_name");
    // Cap to the schema's contributors maxLength (200) so a legacy/over-long
    // remembered name can't push the exported _meta past validation.
    if (remembered && remembered.trim()) state.projectMeta.contributors.translator = remembered.slice(0, 200);
  }
}

// Fill a language <select> with the canonical PROJECT_LANGUAGES + a leading
// empty option (its label conveys the empty meaning per field).
function populateLanguageSelect(sel, emptyLabel) {
  if (!sel) return;
  sel.innerHTML = "";
...
// Set a language select's value, preserving a loaded/inferred value that isn't
// in the canonical list (e.g. a package _meta with en-US / pt-BR) rather than
// silently dropping it.
function setLangSelectValue(sel, value) {
  if (!sel) return;
  // Drop any previously-injected non-standard option so they don't accumulate
  // across loads of different packages.
  Array.prototype.slice.call(sel.options).forEach((o) => {
    if (o.dataset && o.dataset.nonstd === "1") sel.removeChild(o);
  });
  const v = (value == null) ? "" : String(value);
  if (v && !Array.prototype.some.call(sel.options, (o) => o.value === v)) {
    const o = document.createElement("option");
    o.value = v;
    o.textContent = v + " (non-standard)";
    o.dataset.nonstd = "1";
    sel.appendChild(o);
  }
  sel.value = v;
...
function wireProjectMetaPanel() {
  populateLanguageSelect(els.pmSourceLang, "— select source —");
  populateLanguageSelect(els.pmTargetLang, "— none (edit-only) —");
  if (els.pmProjectName) els.pmProjectName.addEventListener("input", () => pmSet("project_name", els.pmProjectName.value));
  if (els.pmSourceLang) els.pmSourceLang.addEventListener("change", () => { pmSet("source_language", els.pmSourceLang.value); updateProjectMetaStatus(); });
  if (els.pmTargetLang) els.pmTargetLang.addEventListener("change", () => { pmSetTarget(els.pmTargetLang.value); updateProjectMetaStatus(); });
  if (els.pmWorkflowMode) els.pmWorkflowMode.addEventListener("change", () => {
    pmSet("workflow_mode", els.pmWorkflowMode.value);
    pmSetTarget(els.pmTargetLang ? els.pmTargetLang.value : ""); // re-coerce target per new mode
    renderProjectMetaPanel();
  });
  if (els.pmDeadline) els.pmDeadline.addEventListener("change", () => pmSet("deadline", els.pmDeadline.value || null));
  if (els.pmTranslator) els.pmTranslator.addEventListener("input", () => {
    pmSetContributor("translator", els.pmTranslator.value);
    renderTranslatorRemember();
  });
}

function pmSet(key, val) {
  if (!state.projectMeta) state.projectMeta = pmEmptyMeta();
  if (val === "" && key === "deadline") { state.projectMeta[key] = null; }
  else { state.projectMeta[key] = val; }
  updateProjectMetaStatus();
}
function pmSetTarget(val) {
  if (!state.projectMeta) state.projectMeta = pmEmptyMeta();
  const mode = state.projectMeta.workflow_mode;
  const t = String(val || "").trim();
  // edit-only allows null target; other modes keep the (possibly empty) string
  // so the validity hint can flag a still-required target.
  state.projectMeta.target_language = (mode === "edit-only" && t === "") ? null : t;
}
function pmSetContributor(role, val) {
  if (!state.projectMeta) state.projectMeta = pmEmptyMeta();
  if (!state.projectMeta.contributors) state.projectMeta.contributors = {};
  const v = String(val || "").trim();
  state.projectMeta.contributors[role] = v === "" ? null : v;
}

function pmDateForInput(deadline) {
  if (!deadline) return "";
  const m = String(deadline).match(/^(\d{4}-\d{2}-\d{2})/);
  return m ? m[1] : "";
}

// Light live-validity hint (full schema validate runs at export). Mirrors the
// schema's user-facing requireds + the edit-only/target oneOf.
function pmValidity() {
  const m = state.projectMeta;
  if (!m) return { valid: true, hint: "" };
  // Single source of truth: validate EXACTLY what the export gate validates
  // (pmFinalizeForExport + ProjectMeta.validate), so the panel hint can never
  // disagree with whether _meta.json actually exports. (audit A-class: a light
  // presence-only check let an over-long translator/project name pass the hint
  // but get silently dropped at export.)
  if (typeof ProjectMeta !== "undefined") {
    let fin;
    try { fin = pmFinalizeForExport(); } catch (e) { return { valid: false, hint: "error finalizing" }; }
    if (fin) {
      const vr = ProjectMeta.validate(fin);
      if (vr.valid) return { valid: true, hint: "" };
      const first = vr.errors && vr.errors[0];
      const where = (first && (first.path || first.message)) || "incomplete";
      return { valid: false, hint: "incomplete (" + where + ")" };
    }
  }
  // Fallback (lib unavailable): light presence check.
  if (!m.project_name || !String(m.project_name).trim()) return { valid: false, hint: "project name required" };
  if (!m.source_language || !String(m.source_language).trim()) return { valid: false, hint: "source language required" };
  const mode = m.workflow_mode || "translation";
  if (mode !== "edit-only" && (m.target_language == null || !String(m.target_language).trim())) {
    return { valid: false, hint: "target language required for " + mode };
  }
  return { valid: true, hint: "" };
}

function updateProjectMetaStatus() {
  if (!els.projectMetaStatus) return;
  if (!state.projectMeta) { els.projectMetaStatus.textContent = "No package loaded"; return; }
  let msg = state.projectMetaFromPackage
    ? "From package _meta.json — edits export with the package"
    : "Inferred from package name — please review before export";
  const v = pmValidity();
  if (!v.valid) msg += "  ⚠ " + v.hint;
  els.projectMetaStatus.textContent = msg;
}

function renderProjectMetaPanel() {
  const has = !!state.projectMeta;
  if (els.projectMetaFields) els.projectMetaFields.classList.toggle("hidden", !has);
  updateProjectMetaStatus();
  if (!has) return;
  const m = state.projectMeta;
  const c = m.contributors || {};
  if (els.pmProjectName) els.pmProjectName.value = m.project_name || "";
  setLangSelectValue(els.pmSourceLang, m.source_language);
  setLangSelectValue(els.pmTargetLang, m.target_language);
  if (els.pmWorkflowMode) els.pmWorkflowMode.value = m.workflow_mode || "translation";
  if (els.pmDeadline) els.pmDeadline.value = pmDateForInput(m.deadline);
  if (els.pmTranslator) els.pmTranslator.value = c.translator || "";
  if (els.pmDesigner) els.pmDesigner.value = c.designer || "";
  if (els.pmProjectId) els.pmProjectId.textContent = m.project_id ? ("id: " + m.project_id) : "";
  renderTranslatorRemember();
}

// One-time-consent remember + hover-forget for operator_name (per panel UX:
// the name is the one reverse-intuitive store, so it gets an explicit but
// minimal inline affordance — a single link, never a button row).
function renderTranslatorRemember() {
  const box = els.pmTranslatorRemember;
  if (!box) return;
  box.textContent = "";
  if (typeof LocalPrefs === "undefined") return;
  let current = (state.projectMeta && state.projectMeta.contributors && state.projectMeta.contributors.translator) || "";
  current = String(current).trim();
  let remembered = LocalPrefs.get("operator_name");
  remembered = remembered ? String(remembered).trim() : "";
  if (remembered && remembered === current) {
    const span = document.createElement("span");
    span.textContent = "Remembered · ";
    const a = document.createElement("a");
    a.href = "#"; a.textContent = "forget"; a.className = "pm-link";
    a.addEventListener("click", (e) => { e.preventDefault(); LocalPrefs.remove("operator_name"); renderTranslatorRemember(); });
    box.appendChild(span); box.appendChild(a);
  } else if (current) {
    const a2 = document.createElement("a");
    a2.href = "#"; a2.textContent = "remember this name"; a2.className = "pm-link";
    a2.addEventListener("click", (e) => { e.preventDefault(); LocalPrefs.set("operator_name", current.slice(0, 200)); renderTranslatorRemember(); });
    box.appendChild(a2);
  }
}
...
async function loadFolderFromServer(dirPath, options) {
  ...
  try {
    ...
    if (silent) {
      await loadPackage(virtualFiles);                 // boot auto-load: no modal
    } else {
      var ir = await loadPackageInteractive(virtualFiles);
      if (ir && ir.cancelled) {
        return { ok: false, reason: "cancelled", path: dirPath };
      }
    }
    renderAll();
...
async function onPackageInputChange(event) {
  ...
  maybePromptDownloadMissingPdfJs();
  try {
    const r = await loadPackageInteractive(files);
...
async function onPackageZipInputChange(event) {
  ...
  try {
    ...
    const r = await loadPackageInteractive(unpacked);
    if (r && r.cancelled) return;
    renderAll();
  } catch (err) {
    console.error(err);
...
async function onSavePackageZip() {
  if (typeof window.ZipCore === "undefined") {
    alert("ZipCore not loaded — check that vendor/fflate.umd.js and lib/zip_core.js are present.");
    return;
  }
  ...
  // Export confirm step: review/edit metadata; Cancel aborts the export.
  if (await showProjectMetaModal("export") === "cancel") return;
  ...
  buildPackageZipAndDownload()
    .then(function (info) {
      ...
      if (info.metaIncluded) {
        body += "\n  - _meta.json (project metadata)";
      } else if (info.metaOriginalKept) {
        body += "\n\n⚠ Your Project-tab edits were NOT saved: " + info.metaSkippedReason +
          ".\n  The package's existing _meta.json was kept unchanged.";
      } else if (info.metaSkippedReason) {
        body += "\n\n⚠ _meta.json NOT included: " + info.metaSkippedReason +
          "\n  Fill the required fields in the Project tab to include it.";
      }
      alert(body);
    })
    .catch(function (err) {
...
// Path-aware finder: among files whose basename matches, return the one at the
// SHALLOWEST archive path (the package root) — so a future nested copy
// (versions/<v>/<name>) is never mistaken for the root file. For A3a' (no
// nesting yet) this is equivalent to findFile, but it's the correct shape and
// keeps the root-vs-nested distinction the version phase will need.
function findFileAtRoot(files, filename) {
  const name = String(filename).toLowerCase();
  let best = null;
  for (let i = 0; i < files.length; i++) {
    const f = files[i];
    if (safeStr(f.name).toLowerCase() !== name) continue;
    const rel = String(f.webkitRelativePath || f.name);
    const depth = rel.split("/").length;
    if (best === null || depth < best.depth) best = { f: f, depth: depth };
  }
  return best ? best.f : null;
}

// Finalize state.projectMeta for export. Read-preserve: clone EVERY field the
// working meta carries (incl contributors.designer set by InDesign), then fill
// machinery fields + canonicalize BCP 47. Never builds a fresh object.
function pmFinalizeForExport() {
  if (typeof ProjectMeta === "undefined" || !state.projectMeta) return null;
  const src = state.projectMeta;
  let m = {};
  Object.keys(src).forEach((k) => { m[k] = src[k]; });
  if (!m.schema_version) m.schema_version = ProjectMeta.CURRENT_SCHEMA_VERSION;
  if (!m.project_id && m.project_name) m.project_id = ProjectMeta.generateProjectId(m.project_name);
  const t = ProjectMeta.now();
  if (!m.created_at) m.created_at = t;
  m.updated_at = t;
  if (m.deadline === "" || m.deadline === undefined) delete m.deadline;
  if (m.target_language === "") m.target_language = (m.workflow_mode === "edit-only") ? null : "";
  try { m = ProjectMeta.canonicalizeBcp47Fields(m); } catch (e) { /* leave as typed */ }
  return m;
}

// Serialize state.projectMeta into _meta.json bytes. Returns {bytes} when valid,
// or {skippedReason} so the caller can SURFACE the skip (not drop it silently).
function buildProjectMetaBytes() {
  if (typeof ProjectMeta === "undefined") return { skippedReason: "metadata lib unavailable" };
  if (!state.projectMeta) return { skippedReason: "no metadata" };
  let m;
  try { m = pmFinalizeForExport(); }
  catch (e) { return { skippedReason: "finalize error: " + e.message }; }
  if (!m) return { skippedReason: "no metadata" };
  const vr = ProjectMeta.validate(m);
  if (!vr.valid) {
    const first = vr.errors && vr.errors[0];
    const where = (first && (first.path || first.message)) || "incomplete";
    console.warn("[project_meta] _meta invalid at export, _meta.json NOT written:", vr.errors);
    return { skippedReason: "metadata incomplete (" + where + ")" };
  }
  return { bytes: new TextEncoder().encode(ProjectMeta.serialize(m)) };
}
...
async function buildPackageZipAndDownload() {
  ...
  // Derive archive root from segments.json's (root) webkitRelativePath.
  let rootDir = "";
  const segFile = findFileAtRoot(state.packageFiles, "segments.json");
  ...
  // Outputs keyed by their EXACT root-relative archive path (path-aware, NOT
  // basename): a root file is replaced ONLY at <root>/<name>, so a nested copy
  // (e.g. a future versions/<v>/translations.json) is never clobbered.
  const outputByPath = {};
  outputByPath[rootDir + "translations.json"]         = enc.encode(JSON.stringify(translations, null, 2));
  outputByPath[rootDir + "translation_qc_report.txt"] = enc.encode(qcText);
  outputByPath[rootDir + "ai_manual_handoff.txt"]     = enc.encode(aiText);

  // _meta.json — read-preserve serialize (state.projectMeta already carries the
  // designer + every package field, plus the user's edits).
  const metaResult = buildProjectMetaBytes();
  if (metaResult.bytes) outputByPath[rootDir + "_meta.json"] = metaResult.bytes;

  const entries = {};
  const seenPaths = {};
  // Re-add originals; replace bytes only when the EXACT archive path is one of ours.
  for (let i = 0; i < state.packageFiles.length; i++) {
    ...
    if (Object.prototype.hasOwnProperty.call(outputByPath, archivePath)) {
      entries[archivePath] = outputByPath[archivePath];
  ...
  // Append any outputs not already present in the original package.
  const outputPaths = Object.keys(outputByPath);
  for (let j = 0; j < outputPaths.length; j++) {
    const p = outputPaths[j];
    if (!seenPaths[p]) entries[p] = outputByPath[p];
  }

  // If our _meta was skipped (invalid) but the package already had a _meta.json,
  // the re-add loop copied it verbatim. Report that honestly — the user's edits
  // did NOT persist, the prior _meta shipped unchanged (audit A-class: prior
  // message wrongly said "NOT included" while a stale _meta.json shipped).
  const metaOriginalKept = !metaResult.bytes &&
    Object.prototype.hasOwnProperty.call(entries, rootDir + "_meta.json");
  ...
  return {
    ...
    metaIncluded: !!metaResult.bytes,
    metaOriginalKept: metaOriginalKept,
    metaSkippedReason: metaResult.skippedReason || null
  };
}
...
async function loadPackage(files, options) {
  ...
  // Working-state persistence: auto-restore this package's unsaved edits /
  // surface a round-trip conflict / clean-load. Non-fatal; runs after the
  // base state is seeded + merged so restore can win over the package's
  // cardOptions. Spec r3 §6. (translationsPayloadForInit declared above.)
  await reconcileWorkingStateOnLoad(translationsPayloadForInit);
  ...
  // ── A3a' project_meta: seed the working meta from the package _meta.json
  // (read-preserve) or infer. Non-fatal — must not block package loading.
  // Skipped when the interactive load modal already seeded + let the user edit
  // it (metaAlreadySeeded) — re-seeding here would clobber those edits.
  if (!(options && options.metaAlreadySeeded)) {
    try {
      await loadProjectMetaForPackage(state.packageFiles, translationsFile);
    } catch (e) {
      console.warn("[project_meta] load failed:", e);
    }
  }
...
// Read the package _meta.json (read-preserve) when present + valid \u2192 seed the
// working meta from it; else infer from the package context. Designer + any
// other-role field the package recorded is preserved by setProjectMetaFromPackage.
async function loadProjectMetaForPackage(files, translationsFile, folderNameOverride) {
  const ctx = {
    // folderNameOverride lets the pre-load modal seed meta BEFORE loadPackage
    // sets state.packageRoot (so inference still gets the right project name).
    folderName: folderNameOverride || state.packageRoot || state.packageName || "",
    hasTranslationsJson: !!translationsFile
  };
  const metaFile = findFileAtRoot(files, "_meta.json");
  if (metaFile && typeof ProjectMeta !== "undefined") {
    try {
      const raw = await readText(metaFile);
      const parsed = ProjectMeta.parse(raw);
      if (parsed.ok) {
        const vr = ProjectMeta.validate(parsed.data);
        if (!vr.valid) {
          // PRESERVE a parseable-but-not-fully-valid package _meta (e.g. a
          // future contributor role the current schema doesn't model) via
          // read-preserve, instead of inferring and clobbering its designer /
          // stable project_id / unknown fields on export. The panel validity
          // hint surfaces the issue; an unfixable _meta is kept, never replaced.
          // (audit A-class: prior "infer instead" silently discarded + then
          // overwrote the package's real _meta.)
          console.warn("[project_meta] package _meta.json doesn't fully validate \u2014 preserving it (read-preserve), not inferring:", vr.errors);
        }
        setProjectMetaFromPackage(parsed.data);
        renderProjectMetaPanel();
        return;
      }
      console.warn("[project_meta] package _meta.json unparseable \u2014 inferring instead:", parsed.errors);
    } catch (e) {
      console.warn("[project_meta] failed to read package _meta.json \u2014 inferring instead:", e);
    }
  }
  setProjectMetaFromInference(ctx);
  renderProjectMetaPanel();
}

// ── A3 follow-up: metadata modal at load/export ─────────────────────────────
// Interactive load: seed the working meta from the picked files (so the modal
// shows real values), pop the editable metadata modal, then either continue
// the full load (Next) or abort it entirely (Cancel) — restoring the prior
// package's meta so a cancelled load leaves NO half-state. boot auto-load
// (silent) does NOT route here, so it stays modal-free.
async function loadPackageInteractive(files) {
  // Snapshot the currently-loaded package's meta so Cancel can restore it.
  const prevMeta = state.projectMeta;
  const prevFromPkg = state.projectMetaFromPackage;
  const prevConf = state.projectMetaConfidence;
  try {
    const tf = findFile(files, "translations.json");
    await loadProjectMetaForPackage(files, tf, inferPackageRoot(files));
  } catch (e) {
    console.warn("[project_meta] pre-load seed failed:", e);
  }
  const choice = await showProjectMetaModal("load");
  if (choice === "cancel") {
    state.projectMeta = prevMeta;
    state.projectMetaFromPackage = prevFromPkg;
    state.projectMetaConfidence = prevConf;
    renderProjectMetaPanel();
    return { cancelled: true };
  }
  await loadPackage(files, { metaAlreadySeeded: true });
  return { ok: true };
}

// Render the EXISTING project-meta form (#projectMetaFields) inside a modal by
// reparenting it (reuse the whole form + field handlers + validation), then move
// it back to its tab on close. Returns "confirm" | "cancel".
//   mode "load"   → buttons Next / Cancel
//   mode "export" → buttons Download / Cancel
var _pmModalOpen = false;   // re-entrancy guard: only one modal may hold the reparented form
function showProjectMetaModal(mode) {
  return new Promise(function (resolve) {
    // A second modal would reparent the SAME #projectMetaFields, stealing it from
    // the first (whose placeholder then dangles → form lost from the tab). Not
    // reachable via normal UX (the overlay blocks clicks), but guard it so a
    // programmatic double-invoke can't corrupt the form. (impl-audit B.)
    if (_pmModalOpen) { resolve("cancel"); return; }
    const fields = els.projectMetaFields;
    if (!fields) { resolve("confirm"); return; }   // degrade gracefully: no form → don't block the flow
    _pmModalOpen = true;
    const originalParent = fields.parentNode;
    const placeholder = document.createComment("pm-fields-home");
    if (originalParent) originalParent.insertBefore(placeholder, fields);

    renderProjectMetaPanel();          // populate fields from state.projectMeta
    fields.classList.remove("hidden");  // ensure visible inside the modal

    const overlay = document.createElement("div");
    overlay.id = "pmModalOverlay";
    overlay.style.cssText = "position:fixed;inset:0;z-index:100050;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.5);";
    const modal = document.createElement("div");
    modal.style.cssText = "background:var(--panel);color:var(--ink);border:1px solid var(--line);border-radius:8px;padding:20px 22px;width:480px;max-width:92vw;max-height:88vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,0.45);font:13px/1.5 system-ui,sans-serif;";
    const title = document.createElement("div");
    title.style.cssText = "font-weight:600;font-size:15px;margin-bottom:12px;";
    title.textContent = (mode === "load") ? "Review project metadata" : "Confirm metadata before export";
    modal.appendChild(title);
    modal.appendChild(fields);          // ← reparent the live form into the modal
    ...
    function finish(result) {
      ...
      _pmModalOpen = false;
      if (placeholder.parentNode) placeholder.parentNode.insertBefore(fields, placeholder); // ← move form home
      if (placeholder.parentNode) placeholder.remove();
      overlay.remove();
      document.removeEventListener("keydown", onKey, true);
      renderProjectMetaPanel();         // resync the tab
      resolve(result);
    }
    const cancelBtn = _pmModalBtn("Cancel", function () { finish("cancel"); }, false);
    const okBtn = _pmModalBtn((mode === "load") ? "Next" : "Download", function () { finish("confirm"); }, true);
...
function _pmModalBtn(label, onClick, primary) {
  const b = document.createElement("button");
  b.type = "button";
  b.textContent = label;
  b.style.cssText = "padding:7px 14px;border-radius:5px;cursor:pointer;font:inherit;border:1px solid " +
    (primary ? "var(--accent)" : "var(--line)") + ";" +
    (primary ? "background:var(--accent);color:#fff;" : "background:var(--bg);color:var(--ink);");
  b.addEventListener("click", onClick);
  return b;
}

// ════════════════════════════════════════════════════════════════════
// Working-state persistence (vendor/working_state_store.js)
//
// Persists the translator's in-progress edits (translationsByTid +
// manualHotspots + cardOptions) to an INDEPENDENT IndexedDB CAS store so
// they survive a browser refresh. Key = content-hash(segments.json) (the
// package's immutable identity). Spec: audit-logs/webapp_persistence_spec_r3.
//
// This is an ADD-ON layer over loadPackage — it never changes the export
// contract (buildTranslationsOutput / ZIP) or the load contract; it only
// auto-saves edits and auto-restores them around the existing flow.
// ════════════════════════════════════════════════════════════════════

var _wsKeySeg = null;                 // hash(segments.json) — current package key
var _wsBaseTranslationsHash = null;   // IMPL-NOTE-1: hash of raw translations.json at load (stashed, pre-edit)
var _wsBuiltInitialByTid = null;      // IMPL-NOTE-2: built-initial translationsByTid snapshot (diff baseline)
var _wsLastHash = null;               // last CAS hash of the stored snapshot (expectedHash for next write)
var _wsLastContentHash = null;        // hash of last-persisted CONTENT (excl. volatile) — no-op guard
var _wsHasEntry = false;              // whether the store already holds an entry for this key
var _wsEnabled = false;               // true once a package + store are ready to persist
var _wsIsLeader = true;               // leader gate for writes; managed by leader election below
// ── Leader-election state (#17) ──
var _wsLeaderTier = null;             // 'A' Web Locks | 'B' localStorage-heartbeat | 'C' CAS-interlock baseline
var _wsTabId = Math.random().toString(36).slice(2) + "_" + (new Date()).getTime();
var _wsReleaseLock = null;            // Tier A: call to release the held Web Lock
var _wsHeartbeatTimer = null;         // Tier B: leader heartbeat interval
var _wsStaleCheckTimer = null;        // Tier B: non-leader staleness watch
var _wsLeaderChannel = null;          // BroadcastChannel for take-over signalling
var _wsLeaderKeyName = null;          // 'ws_leader:<keySeg>'
var _WS_HB_MS = 2000;                 // Tier B heartbeat period
var _WS_HB_STALE_MS = 6000;           // Tier B: leader considered dead after this
var _wsDebounceTimer = null;
var _wsWriting = false;          // a CAS write is in-flight (single-flight guard)
var _wsPendingFlush = false;     // a flush was requested while writing → run one trailing flush
var _wsUnloadWired = false;
var _WS_DEBOUNCE_MS = 1500;

function _wsAvailable() {
  return typeof WorkingStateStore !== "undefined" && WorkingStateStore
    && typeof SHA256Pure !== "undefined" && typeof ProjectMeta !== "undefined";
}
function _wsLeaderActive() { return _wsIsLeader; }

// ════════════════════════════════════════════════════════════════════
// Multi-tab leader election (#17)
//
// Only the LEADER tab autosaves a given package; non-leader tabs show an
// EXPLICIT read-only/take-over banner (never silent — spec §7). RUNTIME
// tier-detection (NOT hardcoded — file:// secure-context + Web Locks
// availability vary by browser: Firefox file:// has Web Locks, Chrome
// file:// likely does not):
//   A = Web Locks (secure-context) — native exclusive lock + auto-handoff.
//   B = localStorage heartbeat — cross-tab signalling but no Web Locks.
//   C = no cross-tab mechanism → the conservative CAS-interlock baseline
//       (leader=true; a real collision trips _wsOnCasConflict → pause).
// Layered ON TOP of the CAS interlock, which remains the lowest-tier safety
// net under all tiers.
// ════════════════════════════════════════════════════════════════════

function _wsTierDetect() {
  try {
    if (typeof navigator !== "undefined" && navigator.locks &&
        typeof navigator.locks.request === "function" && window.isSecureContext) {
      return "A";
    }
  } catch (e) {}
  var hasChannel = (typeof BroadcastChannel !== "undefined");
  var hasLS = false;
  try { window.localStorage.setItem("_ws_probe", "1"); window.localStorage.removeItem("_ws_probe"); hasLS = true; } catch (e) {}
  if (hasChannel || hasLS) return "B";
  return "C";
}

function _wsStartLeaderElection() {
  if (!_wsKeySeg) { _wsIsLeader = true; return; }
  _wsLeaderKeyName = "ws_leader:" + _wsKeySeg;
  _wsLeaderTier = _wsTierDetect();
  try {
    if (typeof BroadcastChannel !== "undefined") {
      _wsLeaderChannel = new BroadcastChannel("ws_leader_" + _wsKeySeg);
      _wsLeaderChannel.onmessage = _wsOnLeaderMessage;
    }
  } catch (e) { _wsLeaderChannel = null; }

  if (_wsLeaderTier === "A") { _wsIsLeader = false; _wsWebLocksElect(); }
  else if (_wsLeaderTier === "B") { _wsIsLeader = false; _wsHeartbeatElect(); }
  else { _wsIsLeader = true; }   // Tier C: CAS-interlock baseline (no cross-tab signal)
}

function _wsStopLeaderElection() {
  try { if (_wsReleaseLock) _wsReleaseLock(); } catch (e) {}
  _wsReleaseLock = null;
  if (_wsHeartbeatTimer) { clearInterval(_wsHeartbeatTimer); _wsHeartbeatTimer = null; }
  if (_wsStaleCheckTimer) { clearInterval(_wsStaleCheckTimer); _wsStaleCheckTimer = null; }
  try {
    if (_wsLeaderTier === "B" && _wsIsLeader && _wsLeaderKeyName) {
      var cur = _wsLsReadLeader();
      if (cur && cur.tabId === _wsTabId) window.localStorage.removeItem(_wsLeaderKeyName);
    }
  } catch (e) {}
  try { if (_wsLeaderChannel) _wsLeaderChannel.close(); } catch (e) {}
  _wsLeaderChannel = null;
  _wsLeaderTier = null;
  _wsLeaderKeyName = null;
  _wsHideNonLeaderBanner();
}

// ── Tier A: Web Locks ──
function _wsWebLocksElect() {
  var name = _wsLeaderKeyName;
  try {
    navigator.locks.request(name, { mode: "exclusive", ifAvailable: true }, function (lock) {
      if (_wsLeaderKeyName !== name) return;          // package switched while pending → release
      if (lock) {
        _wsBecomeLeader();
        return new Promise(function (resolve) { _wsReleaseLock = resolve; });
      }
      // Held by another tab → we're non-leader; queue to take over when it frees.
      _wsBecomeNonLeader();
      navigator.locks.request(name, { mode: "exclusive" }, function (lock2) {
        if (_wsLeaderKeyName !== name) return;        // stale → release immediately
        _wsBecomeLeader();
        return new Promise(function (resolve) { _wsReleaseLock = resolve; });
      });
    });
  } catch (e) {
    _wsLeaderTier = "B"; _wsHeartbeatElect();          // Web Locks unusable → fall back
  }
}

// ── Tier B: localStorage heartbeat (no CAS in LS → claim-then-verify w/ jitter) ──
function _wsLsReadLeader() {
  try { var v = window.localStorage.getItem(_wsLeaderKeyName); return v ? JSON.parse(v) : null; } catch (e) { return null; }
}
function _wsLsClaimLeader() {
  try { window.localStorage.setItem(_wsLeaderKeyName, JSON.stringify({ tabId: _wsTabId, ts: (new Date()).getTime() })); } catch (e) {}
}
function _wsHeartbeatElect() {
  var cur = _wsLsReadLeader();
  var now = (new Date()).getTime();
  var fresh = cur && ((now - (cur.ts || 0)) < _WS_HB_STALE_MS);
  if (!fresh) {
    _wsLsClaimLeader();
    var jitter = 40 + Math.floor(Math.random() * 80);
    setTimeout(function () {
      if (_wsLeaderKeyName == null) return;           // switched away
      var v = _wsLsReadLeader();
      if (v && v.tabId === _wsTabId) { _wsBecomeLeader(); _wsStartHeartbeat(); }
      else { _wsBecomeNonLeader(); _wsStartStaleWatch(); }
    }, jitter);
  } else {
    _wsBecomeNonLeader(); _wsStartStaleWatch();
  }
}
function _wsStartHeartbeat() {
  if (_wsHeartbeatTimer) clearInterval(_wsHeartbeatTimer);
  _wsHeartbeatTimer = setInterval(function () {
    if (!_wsIsLeader || _wsLeaderKeyName == null) return;
    // A2: re-verify ownership BEFORE re-asserting. If another tab claimed the key
    // (take-over / election race), self-demote within one beat instead of fighting
    // forever — guarantees a single leader converges (no permanent double-leader).
    var cur = _wsLsReadLeader();
    if (cur && cur.tabId !== _wsTabId) {
      clearInterval(_wsHeartbeatTimer); _wsHeartbeatTimer = null;
      _wsBecomeNonLeader(); _wsStartStaleWatch();
      return;
    }
    _wsLsClaimLeader();   // still ours (or key vanished) → refresh heartbeat ts
  }, _WS_HB_MS);
}
function _wsStartStaleWatch() {
  if (_wsStaleCheckTimer) clearInterval(_wsStaleCheckTimer);
  _wsStaleCheckTimer = setInterval(function () {
    if (_wsIsLeader || _wsLeaderKeyName == null) return;
    var cur = _wsLsReadLeader();
    var now = (new Date()).getTime();
    if (!cur || ((now - (cur.ts || 0)) >= _WS_HB_STALE_MS)) {
      clearInterval(_wsStaleCheckTimer); _wsStaleCheckTimer = null;
      _wsHeartbeatElect();                            // leader died → re-elect
    }
  }, _WS_HB_MS);
}

// ── Promotion / demotion ──
function _wsBecomeLeader() {
  _wsIsLeader = true;
  _wsEnabled = true;     // A1: promotion re-arms persistence (clears any prior _wsOnCasConflict pause)
  if (_wsStaleCheckTimer) { clearInterval(_wsStaleCheckTimer); _wsStaleCheckTimer = null; }  // stop non-leader stale-watch
  _wsHideNonLeaderBanner();
  // A1/A2: a promoted tab may hold a STALE _wsLastHash (it never wrote, or lost a CAS
  // race). Re-seed from the store so our next CAS `expected` is fresh, THEN flush edits
  // made while non-leader.
  _wsReseedThenFlush();
}
// Re-read the stored snapshot to refresh _wsLastHash/_wsHasEntry, then flush. Used on
// leader promotion (the tab may hold a stale hash from before it was leader).
function _wsReseedThenFlush() {
  if (!_wsKeySeg || !_wsAvailable()) { _wsScheduleFlush(); return; }
  var key = _wsKeySeg;
  WorkingStateStore.read(key).then(function (res) {
    if (_wsKeySeg !== key) return;                       // package switched meanwhile
    if (res && res.ok && res.exists) { _wsLastHash = res.hash; _wsHasEntry = true; }
    else if (res && res.ok) { _wsLastHash = null; _wsHasEntry = false; }
    _wsScheduleFlush();
  }).catch(function () { if (_wsKeySeg === key) _wsScheduleFlush(); });
}
function _wsBecomeNonLeader() {
  _wsIsLeader = false;
  _wsShowNonLeaderBanner();
}

// Take-over signalling: a non-leader asks the current leader to relinquish.
function _wsOnLeaderMessage(ev) {
  var d = ev && ev.data;
  if (!d || d.key !== _wsKeySeg || d.type !== "relinquish") return;
  if (_wsLeaderTier === "A" && _wsIsLeader && _wsReleaseLock) {
    var rel = _wsReleaseLock; _wsReleaseLock = null;
    _wsBecomeNonLeader();
    try { rel(); } catch (e) {}                       // release Web Lock → requester's queued req fires
    _wsWebLocksElect();                               // re-queue behind the new leader
  } else if (_wsLeaderTier === "B" && _wsIsLeader) {
    _wsIsLeader = false;
    if (_wsHeartbeatTimer) { clearInterval(_wsHeartbeatTimer); _wsHeartbeatTimer = null; }
    try { var c = _wsLsReadLeader(); if (c && c.tabId === _wsTabId) window.localStorage.removeItem(_wsLeaderKeyName); } catch (e) {}
    _wsBecomeNonLeader(); _wsStartStaleWatch();
  }
}
function _wsRequestTakeover() {
  if (_wsIsLeader) return;
  if (_wsLeaderTier === "A") {
    try { if (_wsLeaderChannel) _wsLeaderChannel.postMessage({ type: "relinquish", key: _wsKeySeg }); } catch (e) {}
    // No channel → can't signal; the banner tells the user to close the other tab.
  } else if (_wsLeaderTier === "B") {
    _wsLsClaimLeader();
    try { if (_wsLeaderChannel) _wsLeaderChannel.postMessage({ type: "relinquish", key: _wsKeySeg }); } catch (e) {}
    var jitter = 40 + Math.floor(Math.random() * 80);
    setTimeout(function () {
      if (_wsLeaderKeyName == null) return;
      var v = _wsLsReadLeader();
      if (v && v.tabId === _wsTabId) { _wsBecomeLeader(); _wsStartHeartbeat(); }
    }, jitter);
  }
...
// ── Non-leader banner (explicit, never silent — spec §7) ──
function _wsShowNonLeaderBanner() {
  try {
    var id = "wsLeaderBanner";
    if (document.getElementById(id)) return;
    var bar = document.createElement("div");
    bar.id = id;
    bar.style.cssText = "position:fixed;top:0;left:0;right:0;z-index:100000;background:#1d4e89;color:#fff;font:13px/1.4 system-ui,sans-serif;padding:8px 14px;display:flex;align-items:center;gap:12px;box-shadow:0 2px 8px rgba(0,0,0,0.3);";
    var msg = document.createElement("span");
    msg.style.cssText = "flex:1;";
    // Take-over is actionable when we can signal the current leader: Tier B claims
    // the LS key directly; Tier A needs the BroadcastChannel to ask the Web-Lock
    // holder to release. Tier A WITHOUT a channel can't take over → show guidance,
    // not a dead button (integration-audit B).
    var canTakeover = (_wsLeaderTier === "B") || (_wsLeaderTier === "A" && !!_wsLeaderChannel);
    if (canTakeover) {
      msg.textContent = "This package is being edited in another tab — changes here are NOT auto-saved.";
      var btn = document.createElement("div");
      btn.setAttribute("role", "button");
      btn.setAttribute("tabindex", "0");
      btn.textContent = "Take over editing here";
      btn.style.cssText = "padding:4px 10px;border-radius:4px;border:1px solid #fff;background:transparent;color:#fff;cursor:pointer;white-space:nowrap;";
      btn.addEventListener("click", function () { _wsRequestTakeover(); });
      bar.appendChild(msg); bar.appendChild(btn);
    } else {
      msg.textContent = "This package is being edited in another tab — changes here are NOT auto-saved. Close the other tab, then reload here to edit.";
      bar.appendChild(msg);
    }
    document.body.appendChild(bar);
  } catch (e) {}
}
function _wsHideNonLeaderBanner() {
  try { var b = document.getElementById("wsLeaderBanner"); if (b) b.remove(); } catch (e) {}
}

// The §3 working-state payload from current state. LOSSLESS raw state — NOT
// buildTranslationsOutput (export-shaped + lossy: zeroes todo/empty target,
// drops emphasis/prefill flags). Restore must get back exactly what's here.
function serializeWorkingState() {
  return {
    schema: 1,
    _base_content_hash: _wsKeySeg,
    _base_translations_hash: _wsBaseTranslationsHash,   // IMPL-NOTE-1 stashed value, verbatim
    _saved_at: new Date().toISOString(),
    _touched_tids: _wsComputeTouchedTids(),             // IMPL-NOTE-2 diff vs built-initial
    translationsByTid: state.translationsByTid || {},
    manualHotspots: state.manualHotspots || [],
    cardOptions: { format_aware_soft_break_merge: !!state.formatAwareSoftBreakMerge }
  };
}

// Hash of the REAL content only (excludes volatile _saved_at + derived
// _touched_tids) → stable across re-renders when nothing actually changed,
// so over-triggering _wsScheduleFlush (e.g. from renderAll on navigation) is
// free: identical content → same hash → flush skipped, no timestamp churn.
function _wsContentHash() {
  return WorkingStateStore.hashValue({
    t: state.translationsByTid || {},
    h: state.manualHotspots || [],
    c: { format_aware_soft_break_merge: !!state.formatAwareSoftBreakMerge }
  });
}

// IMPL-NOTE-2: touched = tids whose current row differs from the built-initial
// snapshot (post-buildInitialTranslations, pre-edit). Diff-at-flush is robust
// (no per-mutation-point instrumentation to miss). Canonical per-tid compare
// so key order can't false-positive. Feed-only for the deferred 2A per-tid
// merge; §1 stores the whole map losslessly, so restore is safe regardless.
function _wsComputeTouchedTids() {
  if (!_wsBuiltInitialByTid) return [];
  var cur = state.translationsByTid || {};
  var base = _wsBuiltInitialByTid;
  var seen = {}, out = [], k;
  for (k in cur) if (Object.prototype.hasOwnProperty.call(cur, k)) seen[k] = 1;
  for (k in base) if (Object.prototype.hasOwnProperty.call(base, k)) seen[k] = 1;
  for (k in seen) {
    var a = cur[k] === undefined ? null : cur[k];
    var b = base[k] === undefined ? null : base[k];
    if (WorkingStateStore._internal.canonicalText(a) !== WorkingStateStore._internal.canonicalText(b)) out.push(k);
  }
  return out;
}

// Direct-assign restore (NOT via buildInitialTranslations, which re-derives +
// re-clobbers prefill flags). Rebuild derived indices after (改动4).
function restoreWorkingState(snapshot) {
  if (snapshot.translationsByTid && typeof snapshot.translationsByTid === "object") {
    state.translationsByTid = snapshot.translationsByTid;
  }
  if (Array.isArray(snapshot.manualHotspots)) {
    state.manualHotspots = snapshot.manualHotspots;
  }
  if (snapshot.cardOptions && typeof snapshot.cardOptions === "object") {
    // Restore the user's merge-mode (wins over the package's — restore runs
    // after applyCardOptionsFromPayload). Reuse the package apply path.
    applyCardOptionsFromPayload({ card_options: snapshot.cardOptions });
  }
  rebuildManualHotspotIndex();   // direct-assign left state.manualHotspotById stale
}

// Reconcile hook — called from loadPackage at :1493 (after autoMerge, after
// applyCardOptionsFromPayload). Non-fatal: any store error → clean load.
async function reconcileWorkingStateOnLoad(translationsPayloadForInit) {
  _wsResetSession();
  if (!_wsAvailable()) return;            // no store → clean load, persistence off
  try {
    // IMPL-NOTE-1: capture base translations hash ONCE, HERE, over the RAW
    // payload (pre-buildInitialTranslations). Stashed → written verbatim into
    // every flush. (null payload = no package translations.json yet; hash(null)
    // is a stable sentinel.)
    _wsBaseTranslationsHash = WorkingStateStore.hashValue(
      translationsPayloadForInit === undefined ? null : translationsPayloadForInit);
    // IMPL-NOTE-2: snapshot built-initial translationsByTid as the diff baseline.
    _wsBuiltInitialByTid = JSON.parse(JSON.stringify(state.translationsByTid || {}));
    _wsKeySeg = WorkingStateStore.hashValue(state.segmentsPayload);
  } catch (e) {
    console.warn("[ws] hash/snapshot failed, persistence off:", e);
    return;
  }

  var res;
  try { res = await WorkingStateStore.read(_wsKeySeg); }
  catch (e) { console.warn("[ws] store.read threw, clean load:", e); _wsArm(null, false); return; }
  if (!res || !res.ok) { console.warn("[ws] store.read not ok, clean load:", res && res.errors); _wsArm(null, false); return; }
  if (!res.exists) { _wsArm(null, false); return; }   // no prior → clean load, armed to save

  var entry = res.snapshot || {};
  if (entry._base_translations_hash === _wsBaseTranslationsHash) {
    // Same package re-opened with unsaved working state → auto-restore.
    try { restoreWorkingState(entry); } catch (e) { console.warn("[ws] restore failed:", e); }
    _wsArm(res.hash, true);
    return;
  }
  // Package translations differ from the snapshot's base (a round-trip brought
  // back reviewed translations). Never silently clobber either side — surface a
  // 3-way choice (irreversible "drop" shown neutrally, no leading default).
  var choice = "package";
  try { choice = await surfaceWorkingStateConflict(entry); }
  catch (e) { console.warn("[ws] conflict surface failed, keeping package:", e); choice = "package"; }
  if (choice === "local") {
    try { restoreWorkingState(entry); } catch (e) { console.warn("[ws] restore failed:", e); }
    _wsArm(res.hash, true);
  } else if (choice === "package") {
    // Keep freshly-loaded package state; next flush CAS-overwrites the stored
    // snapshot (expected = res.hash, so a concurrent tab still casErrors).
    _wsArm(res.hash, true);
  } else {
    // cancel → keep package state but do NOT auto-overwrite the stored local
    // edits; leave autosave OFF this session so we never clobber them.
    _wsEnabled = false;
  }
}

function _wsResetSession() {
  _wsStopLeaderElection();   // release the prior package's lock/heartbeat/channel first
  _wsKeySeg = null; _wsBaseTranslationsHash = null; _wsBuiltInitialByTid = null;
  _wsLastHash = null; _wsLastContentHash = null; _wsHasEntry = false; _wsEnabled = false;
  _wsWriting = false; _wsPendingFlush = false;   // a prior package's in-flight write self-heals on its own .then
  if (_wsDebounceTimer) { clearTimeout(_wsDebounceTimer); _wsDebounceTimer = null; }
}

function _wsArm(lastHash, hasEntry) {
  _wsLastHash = lastHash;       // null = create-only on first write
  _wsHasEntry = hasEntry;
  try { _wsLastContentHash = _wsContentHash(); } catch (e) { _wsLastContentHash = null; }
  _wsEnabled = true;
  _wsWireUnloadFlush();
  _wsStartLeaderElection();     // #17: only the leader tab actually writes
}

// (Re)arm the debounce after an edit. Liveness optimization; the unload flush
// is the completeness GUARANTEE (an un-instrumented edit path still persists
// on tab-hide / close). Cheap to over-call — the content-hash guard in _wsFlush
// makes a no-change flush a no-op.
function _wsScheduleFlush() {
  if (!_wsEnabled) return;
  if (_wsDebounceTimer) clearTimeout(_wsDebounceTimer);
  _wsDebounceTimer = setTimeout(function () { _wsDebounceTimer = null; _wsFlush(false); }, _WS_DEBOUNCE_MS);
}

// Flush current working state via CAS. isUnload=true is the best-effort
// unload path (fire-and-forget; can't await before the page is gone).
//
// Writes are SINGLE-FLIGHTED (_wsWriting): a flush requested while a write is
// in-flight coalesces into ONE trailing flush (_wsPendingFlush) that runs in
// the settle handler with the freshly-committed _wsLastHash. Without this, two
// overlapping writes (debounce + unload, or a slow IDB write + the next
// debounce) both read the stale _wsLastHash as `expected`; the second loses the
// CAS to the first and the self-inflicted casError would wrongly pause autosave
// for the whole session (impl-audit A-class). _wsLastContentHash is committed
// only on SUCCESS, so a failed write naturally retries on the next change.
function _wsFlush(isUnload) {
  if (!_wsEnabled || !_wsLeaderActive()) return;
  if (_wsWriting) { _wsPendingFlush = true; return; }   // serialize → trailing flush
  if (_wsDebounceTimer) { clearTimeout(_wsDebounceTimer); _wsDebounceTimer = null; }
  var contentHash;
  try { contentHash = _wsContentHash(); }
  catch (e) { console.warn("[ws] content hash failed:", e); return; }
  if (contentHash === _wsLastContentHash) return;   // nothing actually changed → skip
  var payload;
  try { payload = serializeWorkingState(); }
  catch (e) { console.warn("[ws] serialize failed:", e); return; }
  var expected = _wsHasEntry ? _wsLastHash : null;  // null/absent = create-only first write
  _wsWriting = true;
  WorkingStateStore.write(_wsKeySeg, payload, expected).then(function (r) {
    _wsWriting = false;
    if (r && r.ok) {
      _wsLastHash = r.newHash || _wsLastHash;
      _wsHasEntry = true;
      _wsLastContentHash = contentHash;   // commit only on success
      _wsRunPendingFlush();
    } else if (r && r.casError) {
      _wsOnCasConflict();                 // genuine cross-tab conflict (self-race now impossible)
    } else {
      _wsNotifyFlushFailure(r && r.errors);
      _wsRunPendingFlush();              // not a conflict → retry the latest state
    }
  }).catch(function (e) {
    _wsWriting = false;
    if (!isUnload) _wsNotifyFlushFailure([String(e)]);
    _wsRunPendingFlush();
  });
}

// Run the coalesced trailing flush, if one was requested while a write was
// in-flight. Recomputes against current state with the now-fresh _wsLastHash.
function _wsRunPendingFlush() {
  if (_wsPendingFlush) { _wsPendingFlush = false; _wsFlush(false); }
}

// Another tab/window wrote since our last read (CAS mismatch). Until proper
// leader election (#17) we conservatively STOP autosaving this session so we
// never silently overwrite the other tab's edits (no last-write-wins), and
// surface it visibly. (Leader election will make this near-impossible — only
// the leader writes.)
function _wsOnCasConflict() {
  _wsEnabled = false;
  _wsBanner("This package is open in another tab. Auto-save is paused here to avoid overwriting the other tab's edits. Close the other tab and reload to resume.", "warn");
}

function _wsNotifyFlushFailure(errors) {
  console.error("[ws] auto-save failed:", errors);
  _wsBanner("Auto-save failed — your recent edits are NOT persisted. " + (errors && errors.length ? "(" + errors.join("; ") + ")" : ""), "error");
}

// Wire the unload-flush ONCE (idempotent). visibilitychange→hidden is the
// reliable mobile/tab-switch path (beforeunload doesn't fire there); both run
// a best-effort sync flush. Spec §5.
function _wsWireUnloadFlush() {
  if (_wsUnloadWired) return;
  _wsUnloadWired = true;
  window.addEventListener("beforeunload", function () {
    try { _wsFlush(true); } catch (e) {}
    // Tier B: release our leader claim promptly so another tab takes over without
    // waiting out the staleness window. (Tier A's Web Lock auto-releases on unload.)
    try {
      if (_wsLeaderTier === "B" && _wsIsLeader && _wsLeaderKeyName) {
        var c = _wsLsReadLeader();
        if (c && c.tabId === _wsTabId) window.localStorage.removeItem(_wsLeaderKeyName);
      }
    } catch (e) {}
  });
  document.addEventListener("visibilitychange", function () {
    if (document.visibilityState === "hidden") { try { _wsFlush(true); } catch (e) {} }
  });
}

// Minimal visible banner (flush failures / multi-tab notices must be visible,
// not silent — spec §5). Fixed top strip, manual dismiss.
function _wsBanner(msg, kind) {
  try {
    var id = "wsBanner";
    ...
    var bar = document.createElement("div");
    bar.id = id;
    var bg = kind === "error" ? "#b00020" : (kind === "warn" ? "#8a6d00" : "#333");
    bar.style.cssText = "position:fixed;top:0;left:0;right:0;z-index:100000;background:" + bg +
      ";color:#fff;font:13px/1.4 system-ui,sans-serif;padding:8px 40px 8px 14px;box-shadow:0 2px 8px rgba(0,0,0,0.3);";
    bar.textContent = msg;
    var x = document.createElement("div");
    x.textContent = "×";
    x.style.cssText = "position:absolute;top:6px;right:12px;cursor:pointer;font-size:18px;line-height:1;";
...
// 3-way conflict modal → resolves "local" | "package" | "cancel". Neutral
// layout: the irreversible "discard" option carries no leading highlight
// (project rule: irreversible decisions show options neutrally).
function surfaceWorkingStateConflict(entry) {
  return new Promise(function (resolve) {
    var existing = document.getElementById("wsConflictOverlay");
    if (existing) existing.remove();
    var overlay = document.createElement("div");
    overlay.id = "wsConflictOverlay";
    ...
    modal.style.cssText = "background:var(--panel);color:var(--ink);border:1px solid var(--line);border-radius:8px;padding:22px;width:520px;max-width:92vw;box-shadow:0 8px 32px rgba(0,0,0,0.45);font:13px/1.5 system-ui,sans-serif;";
    var savedAt = (entry && entry._saved_at) ? entry._saved_at : "unknown time";
    var title = document.createElement("div");
    title.style.cssText = "font-weight:600;font-size:15px;margin-bottom:10px;";
    title.textContent = "Unsaved local edits vs. updated package";
    var body = document.createElement("div");
    body.style.cssText = "margin-bottom:16px;";
    body.innerHTML = "This package has unsaved edits in this browser (last auto-saved " +
      "<b>" + _wsEsc(savedAt) + "</b>), but the package you just opened contains " +
      "<b>different translations</b> (e.g. a reviewed round-trip). Choose how to proceed:";
    var btns = document.createElement("div");
    btns.style.cssText = "display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end;";
    function mkBtn(label, val, danger) {
      var b = document.createElement("button");
      b.textContent = label;
      b.style.cssText = "padding:8px 12px;border-radius:5px;border:1px solid var(--line);cursor:pointer;font:inherit;" +
        "background:var(--bg);color:var(--ink);";   // neutral both branches (irreversible → no highlighted destructive)
      b.addEventListener("click", function () { overlay.remove(); resolve(val); });
      return b;
    }
    // Neutral order: keep-local, use-package (discard), cancel. No default focus
    // on the destructive option.
    btns.appendChild(mkBtn("Keep my local edits", "local", false));
    btns.appendChild(mkBtn("Use package translations (discard local)", "package", true));
    btns.appendChild(mkBtn("Cancel", "cancel", false));
    modal.appendChild(title); modal.appendChild(body); modal.appendChild(btns);
    overlay.appendChild(modal);
    document.body.appendChild(overlay);
  });
}
...
function buildInitialTranslations(segments, translationsPayload, templatePayload) {
  ...
  fromPayload.forEach(row => {
    ...
    // Carry suggested/confirmed emphasis runs across reload. normalizeTranslations
    // Payload already sanitized them; gate to non-control / non-empty rows
    // (mirrors export) since offsets can't align otherwise. Offsets index the
    // visible-token target form, which round-trips identically here.
    if (!isControlOnly && !effectiveEmpty &&
        Array.isArray(row.target_emphasis_runs) && row.target_emphasis_runs.length) {
      out[row.tid].target_emphasis_runs = row.target_emphasis_runs;
      if (row.target_emphasis_runs_auto === true) out[row.tid].target_emphasis_runs_auto = true;
      if (row.target_emphasis_runs_user_set === true) out[row.tid].target_emphasis_runs_user_set = true;
      // #MTCARRY HOLE-B: do NOT trust an inbound `faithful` flag from loaded
      // JSON. The codec (applyDevTranslateRows) is the ONLY legitimate producer
      // of target_emphasis_runs_faithful; an external/hand-edited/old-build
      // progress.json must not be able to declare faithful and smuggle
      // fabricated positions past the client import gate. Faithful is stripped at
      // this import boundary; a genuine in-session codec carry re-establishes
      // it (and is re-derived at host re-import). The flag is intentionally
      // NOT copied here.
    }
  });
...
const VALID_FORMAT_ACTIONS = ["bold", "italic", "underline", "strikethrough", "superscript", "subscript", "color", "size", "link"];
...
function isValidAnnotation(a) {
  ...
  if (t === "format") {
    ...
    if (action === "size"  && !(typeof a.size === "number" && a.size > 0)) return false;
...
function normalizeAnnotationItem(a) {
  ...
  if (t === "format") {
    return {
      ...
      _auto: a._auto === true ? true : undefined
    };
  }
...
// Sanitize target_emphasis_runs from a (possibly hand-edited / round-tripped)
// translations.json. Keep only entries the pipeline can consume: numeric
// start<end + a diff object. Mirrors the {start,end,diff} shape applyOnePara
// reads (indesign-toolkit v2_pipeline.js step 3.4).
function normalizeEmphasisRuns(arr) {
  if (!Array.isArray(arr)) return [];
  const out = [];
  for (let i = 0; i < arr.length; i++) {
    ...
    if (!r.diff || typeof r.diff !== "object") continue;
    const o = { start: r.start, end: r.end, diff: r.diff };
    if (r.confidence) o.confidence = safeStr(r.confidence);
    out.push(o);
  }
  return out;
}
...
function normalizeTranslationsPayload(payload) {
  ...
  return rows.map(r => {
    ...
    // Carry target_emphasis_runs + its flags across package reload. Without
    // this the confirm/suggestion state survives autosave (lossless) but is
    // dropped when a translations.json round-trips back in, silently undoing
    // a translator's Accept. _auto vs user_set decide pipeline application.
    const runs = normalizeEmphasisRuns(r && r.target_emphasis_runs);
    if (runs.length) {
      obj.target_emphasis_runs = runs;
      if (r && r.target_emphasis_runs_auto === true) obj.target_emphasis_runs_auto = true;
      if (r && r.target_emphasis_runs_user_set === true) obj.target_emphasis_runs_user_set = true;
      // #MTCARRY HOLE-B: external trust boundary. `faithful` is deliberately
      // NOT carried through from loaded JSON — only an in-session codec run may
      // set it (see applyDevTranslateRows). This stops a crafted/old-build
      // progress.json from bypassing the client import gate via a fabricated
      // faithful flag. `auto`/`user_set` pass through (they don't grant the
      // gate exemption on their own).
    }
    return obj;
  });
}
...
function renderAll() {
  ...
  renderPageControls();
  renderPdf();
  // Catch-all auto-save trigger: every edit re-renders, so this covers edit
  // paths not individually instrumented. Free for non-edits (content-hash
  // guard skips a no-change flush). Liveness only — unload flush guarantees.
  _wsScheduleFlush();
}
...
function renderSegmentsList(items) {
  ...
  const html = rows.map(seg => {
    ...
    // Runs are clipped+re-based to this segment's own range so soft-break
    // tails don't inherit a sibling's emphasis (see localEmphasisRunsForSeg).
    const sourceRuns  = localEmphasisRunsForSeg(seg);
...
function buildPageCardHtml(seg) {
  ...
  const targetReadonlyTip = isControlOnly
    ...
    : (isMergeTailRow(tr)
        ...
        : "Enter translation...");
  // Phase 8B/8C: render emphasis runs inline when present. Clipped+re-based to
  // this segment's own range (soft-break tails must not inherit sibling runs).
  const cardSourceRuns  = localEmphasisRunsForSeg(seg);
...
function onEmphasisSpanClick(spanEl) {
    ...
    if (!seg) return;
    // Index into the SAME clipped+re-based run array the renderer tagged with
    // data-emp-idx — using the raw parent-global array here would resolve the
    // wrong run (and a wrong source slice) for soft-break tails.
    var localRuns = localEmphasisRunsForSeg(seg);
    if (!localRuns) return;
    var run = localRuns[idx];
...
function sourceColorForRange(seg, offset, length) {
    ...
    // `offset` is local to seg.source_text (source_links offsets are local —
    // see onSourceLinkSpanClick / commitFormatPaintLink), so compare against the
    // clipped+re-based runs, not the raw parent-global emphasis_runs.
    var runs = localEmphasisRunsForSeg(seg);
    if (!runs) return null;
...
function diffToAnnotationEntries(diff, offset, length) {
    ...
    // InDesign also encodes super/subscript via the `position` enum
    // independent of baseline_shift (client footnotes use this path:
    // diff = {position: "SUPERSCRIPT"} with baseline_shift unset). Map
    // both representations to the same annotation actions.
    if (diff.position === "SUPERSCRIPT") {
        anns.push({ type: "format", action: "superscript", offset: offset, length: length });
    } else if (diff.position === "SUBSCRIPT") {
        anns.push({ type: "format", action: "subscript", offset: offset, length: length });
    }
    if (diff.fillColor) {
...
function commitFormatPaint(tid, run, ss, se) {
    ...
    // Fill required text + context fields so the annotation survives
    // save→reload via isValidAnnotation/normalizeAnnotations (same pattern
    // as createAnnotation:8090+). Without this, type=format annotations
    // are reload-time rejected by the !safeStr(a.text) gate.
    var paintTxt = safeStr(row.target_text);
    if (ss >= 0 && se > ss && se <= paintTxt.length) {
        var paintCtx = buildAnnotationContext(paintTxt, ss, se - ss);
        for (var __pi = 0; __pi < newAnns.length; __pi += 1) {
            newAnns[__pi].text = paintTxt.substring(ss, se);
            newAnns[__pi].context_before = paintCtx.context_before;
            newAnns[__pi].context_after = paintCtx.context_after;
        }
    }
...
function updateTargetText(tid, value, options) {
  ...
  var prevTargetText = row.target_text;
  row.target_text = safeStr(value);
  var targetTextChanged = row.target_text !== prevTargetText;
  if (opts.fromUser) {
    row.target_auto_prefill = false;
  }
  _wsScheduleFlush();   // typing is the dominant edit path — arm autosave directly
  if (hasEffectiveTargetText(row.target_text)) {
    ...
    // Format propagation is faithful MT-carry only (decodeEmphasisFromGoogle →
    // applyDevTranslateRows), set at translate time. The webapp no longer
    // generates heuristic target_emphasis_runs on keystroke — translators edit
    // content; format rides the translation (design-intent.md §5).
    //
    // A MANUAL edit changes the text out from under any machine-carried run:
    // its {start,end} index the PRE-edit string, so keeping them — above all
    // target_emphasis_runs_faithful, which exempts them from the client import
    // gate — would mis-position emphasis on export (violates the §3/§4 红线
    // "绝不错位"). Drop the whole carry, mirroring the empty-target branch
    // below; re-running MT re-establishes a faithful carry at correct offsets.
    // applyDevTranslateRows calls updateTargetText(fromUser) BEFORE installing
    // its fresh carry, so a genuine in-session carry survives this drop.
    if (opts.fromUser && targetTextChanged && Array.isArray(row.target_emphasis_runs)) {
      delete row.target_emphasis_runs;
      delete row.target_emphasis_runs_auto;
      delete row.target_emphasis_runs_user_set;
      delete row.target_emphasis_runs_faithful;
    }
  } else {
    if (row.status !== "skip") {
      row.status = "todo";
    }
    // Empty target → drop any carried emphasis runs (offsets index a now-gone
    // string; a later MT translate re-derives them faithfully).
    if (Array.isArray(row.target_emphasis_runs)) {
      delete row.target_emphasis_runs;
      delete row.target_emphasis_runs_auto;
      delete row.target_emphasis_runs_user_set;
      delete row.target_emphasis_runs_faithful;
...
function getDevTranslateRows() {
  var __empBridgeStats = { totalRows: 0, withSrcRuns: 0, sampleTids: [] };
  var __result = state.segments.map(seg => {
    ...
    // Pass source-side emphasis runs (bold/italic/color/superscript/etc.
    // ranges captured at extraction time) to dev_translate so it can wrap
    // them with position-preserving markers around Google Web translate.
    // Decoded back to target_emphasis_runs on return — see decodeEmphasis
    // in dev_translate.js. Only emitted for non-merge-head rows; merge_head
    // would need its own combined-emphasis derivation which is out of scope.
    // Clip to this segment's own range — the markers get wrapped into the LOCAL
    // source_text at run offsets, so a soft-break tail's runs must be re-based
    // (and a sibling's runs dropped) or markers land at the wrong positions.
    var srcRuns = null;
    if (!isMergeHead) {
      var localSrcRuns = localEmphasisRunsForSeg(seg);
      if (localSrcRuns && localSrcRuns.length > 0) {
        srcRuns = localSrcRuns;
      }
    }
    __empBridgeStats.totalRows += 1;
    if (srcRuns) {
      __empBridgeStats.withSrcRuns += 1;
      if (__empBridgeStats.sampleTids.length < 3) {
        __empBridgeStats.sampleTids.push(tid + " (" + srcRuns.length + " runs)");
      }
    }
    return {
      tid: tid,
      source_text: sourceText,
      source_emphasis_runs: srcRuns,
  ...
  console.log("[emp-prsv] bridge.getRows: " + __empBridgeStats.totalRows +
    " rows total, " + __empBridgeStats.withSrcRuns +
    " carry source_emphasis_runs. samples: " +
    (__empBridgeStats.sampleTids.join(", ") || "(none)"));
  return __result;
}
...
function applyDevTranslateRows(rows, options) {
  ...
  for (i = 0; i < list.length; i += 1) {
    ...
    row = state.translationsByTid[tid];
    // If dev_translate decoded position markers and produced direct
    // target_emphasis_runs, install them as the faithful MT-carry. Positions
    // came from Google's translation directly (markers travel with the words),
    // so they land at the semantically-correct offsets. Per cross-repo policy
    // in CLAUDE.md ("user 未手动标注 → pipeline 不沿用源格式"), keep `_auto: true`
    // so a NON-faithful carry stays gated; faithful (below) exempts a clean one.
    if (row && Array.isArray(item.target_emphasis_runs)
        && item.target_emphasis_runs.length > 0) {
      row.target_emphasis_runs = item.target_emphasis_runs;
      row.target_emphasis_runs_auto = true;
      // #MTCARRY option②: grant the FAITHFUL gate-exemption ONLY when the codec
      // reported a PROVABLY-CLEAN marker roundtrip (decode verdict, threaded via
      // the dev_translate bridge as target_emphasis_runs_clean). "Clean" = every
      // marker survived in the exact (id,kind) order encode emitted — no drop,
      // add, id-swap, interleave, or drift PAST another marker. Earlier we tried
      // to DETECT drift after the fact with a length/fraction heuristic; that is
      // root-cause-blind (a lone marker drifting within its own gap leaves no
      // length/structure signal at high source coverage), so it is removed.
      // When the roundtrip is NOT clean, faithful is withheld: the runs stay
      // _auto-only and the client import gate strips them (missing emphasis = safe;
      // a mis-placed run can never auto-apply). The runs still render as a
      // read-only overlay decoration so the translator sees where they landed.
      // faithful is set ONLY here (codec path) — no other producer remains.
      // ⚠ RESIDUAL (handoff): a lone marker drifting WITHIN its own gap (not
      // crossing another marker) preserves the sequence → passes "clean" → still
      // granted faithful. This is post-hoc undetectable; closing it needs a more
      // robust (gap-less) marker encoding — gated on the measured mangling rate.
      if (item.target_emphasis_runs_clean === true) {
        row.target_emphasis_runs_faithful = true;
      }
      // Marker-derived runs are not user-confirmed — clear any stale user_set
      // (e.g. carried in from a prior loaded package) so the overlay renders
      // them as auto-carry, not confirmed.
      delete row.target_emphasis_runs_user_set;
      // Project marker-derived runs onto row.annotations[] so the target
      // overlay's .ann-highlight pipeline renders them in the textarea.
      // Each annotation is tagged `_auto: true` so the InDesign-side
      // export pipeline drops it (per CLAUDE.md cross-repo policy: "user
      // 未手动标注 → pipeline 不沿用源格式"); translator can keep, edit, or
      // delete via Reset / annotation toolbar.
      if (!Array.isArray(row.annotations)) row.annotations = [];
      var __empAnnsAdded = 0;
      var __empTargetTxt = safeStr(target);
      for (var __r = 0; __r < item.target_emphasis_runs.length; __r++) {
        var __run = item.target_emphasis_runs[__r];
        var __empOffset = Number(__run && __run.start);
        var __empEnd = Number(__run && __run.end);
        if (!isFinite(__empOffset) || !isFinite(__empEnd)) continue;
        var __empLen = __empEnd - __empOffset;
        if (__empLen <= 0) continue;
        if (__empOffset < 0 || __empOffset + __empLen > __empTargetTxt.length) continue;
        var __subAnns = diffToAnnotationEntries(__run.diff, __empOffset, __empLen);
        var __empCtx = buildAnnotationContext(__empTargetTxt, __empOffset, __empLen);
        for (var __sa = 0; __sa < __subAnns.length; __sa += 1) {
          // isValidAnnotation requires non-empty `text` for type=format,
          // and the in-memory annotation must survive save→reload via
          // normalizeAnnotations. Fill text + context from the just-set
          // target_text slice (same pattern as createAnnotation:8090+).
          __subAnns[__sa].text = __empTargetTxt.substring(__empOffset, __empOffset + __empLen);
          __subAnns[__sa].context_before = __empCtx.context_before;
          __subAnns[__sa].context_after = __empCtx.context_after;
          __subAnns[__sa]._auto = true;
          row.annotations.push(__subAnns[__sa]);
          __empAnnsAdded += 1;
        }
      }
      console.log("[emp-prsv] bridge.applyRows: installed " +
        item.target_emphasis_runs.length + " marker-derived target runs on " + tid +
        " (+ " + __empAnnsAdded + " annotation entries projected for overlay)");
    }
...
function buildTranslationsOutput() {
  ...
  const rows = state.segments.map(seg => {
    ...
    if (Array.isArray(tr.target_emphasis_runs) && tr.target_emphasis_runs.length > 0
      ...
      // BRIDGE-39 gate: a non-faithful MT-carry exports _auto=true → the import
      // gate strips it (auto && !faithful). A faithful carry additionally
      // exports faithful=true (below) → exempt. user_set is webapp-internal
      // (pipeline ignores it) but is exported so a package round-trip preserves
      // any confirmed state carried in from a loaded package.
      if (tr.target_emphasis_runs_auto) outRow.target_emphasis_runs_auto = true;
      if (tr.target_emphasis_runs_user_set) outRow.target_emphasis_runs_user_set = true;
      // #MTCARRY: a faithful source-carry exports auto=true AND faithful=true.
      // An OLD import build (unaware of `faithful`) sees auto=true and keeps
      // stripping it (safe backward-compatible default); a NEW build reads
      // faithful=true at the BRIDGE-39 gate and applies the carry. The two
      // sides must ship together — the app.js flag alone is a no-op until the
      // import gate (v2_pipeline.js) learns the exemption.
      if (tr.target_emphasis_runs_faithful) outRow.target_emphasis_runs_faithful = true;
    }
    return outRow;
  });
...
function showBulkImportModal(startTid) {
  ...
  var modal = document.createElement("div");
  modal.style.cssText = "background:var(--panel);color:var(--ink);border-radius:8px;padding:20px;width:700px;max-width:92vw;max-height:85vh;display:flex;flex-direction:column;box-shadow:0 8px 32px rgba(0,0,0,0.3);";
  ...
  area.style.cssText = "min-height:120px;resize:vertical;font-size:13px;padding:8px;border:1px solid var(--line);border-radius:4px;background:var(--field-target-bg);color:inherit;font-family:inherit;";
  area.placeholder = "Paste translated paragraphs here, one per line...";

  // Preview table container.
  var previewWrap = document.createElement("div");
  previewWrap.style.cssText = "margin-top:8px;max-height:220px;overflow-y:auto;border:1px solid var(--line);border-radius:4px;font-size:12px;";
  ...
  thead.innerHTML = '<tr style="position:sticky;top:0;background:var(--field-source-bg);z-index:1;">' +
    '<th style="padding:4px 6px;text-align:left;width:30px;border-bottom:1px solid var(--line);">#</th>' +
    '<th style="padding:4px 6px;text-align:left;width:45%;border-bottom:1px solid var(--line);">Source</th>' +
    '<th style="padding:4px 6px;text-align:left;border-bottom:1px solid var(--line);">Target (pasted)</th>' +
    '</tr>';
  ...
  info.style.cssText = "font-size:12px;color:var(--muted);margin-top:6px;";
  info.textContent = "0 paragraphs detected";
...
// Global start offset of each soft-break group member within the parent
// paragraph, in member order. member[i] begins after the sum of all preceding
// members' source_text lengths + their soft-break separator char(s). This is
// the single source of truth for the global-offset arithmetic shared by
// computeSoftBreakGroupEffectiveFormats and localEmphasisRunsForSeg — keeping
// it in one place avoids the two diverging (a forced line break is 1 char;
// some separators carry an explicit multi-char string).
//
// `members` MUST already be sorted by soft_break_index ascending.
function softBreakGroupMemberStarts(members) {
  var starts = [];
  if (!Array.isArray(members)) return starts;
  var globalStart = 0;
...
//      paragraph (softBreakGroupMemberStarts).
...
function computeSoftBreakGroupEffectiveFormats(members) {
  ...
  var out = [];
  var starts = softBreakGroupMemberStarts(members);
  for (var i = 0; i < members.length; i++) {
    var seg = members[i] || {};
...
// Return `seg`'s emphasis_runs clipped to its OWN source range and re-based to
// LOCAL (0-based) offsets. This is the fix for cross-segment emphasis pollution:
// a soft-break sub-segment shares its parent paragraph's format_snapshot, so
// `format_snapshot.emphasis_runs[]` carry GLOBAL parent offsets covering EVERY
// sibling sub-segment. Any consumer that treats {start,end} as offsets into the
// sub-segment's own source_text (source render, emp-click slice, color lookup,
// dev_translate marker wrap) must clip first — or a tail sub-segment (e.g.
// "Preparing for This Seminar") inherits the head's run ("Getting Ready" 25pt
// Semibold) and emphasizes the wrong characters in the tail.
//
// For a non-soft-break segment (or the index-0 head) globalStart is 0, so this
// is an identity transform except it still trims any run belonging to a LATER
// sub-segment (which the head would otherwise pick up).
//
// Returns null when the segment has no emphasis_runs at all (callers already
// branch on a null/empty result). Order is preserved so the i-th returned run
// matches the i-th run the renderer tags with data-emp-idx.
function localEmphasisRunsForSeg(seg) {
  if (!seg || !seg.format_snapshot || !Array.isArray(seg.format_snapshot.emphasis_runs)) {
    return null;
  }
  var runs = seg.format_snapshot.emphasis_runs;
  var localLen = (seg.source_text || "").length;
  var globalStart = 0;
  if (seg.soft_break_group && (seg.soft_break_index || 0) > 0) {
    var gid = safeStr(seg.soft_break_group);
    var members = [];
    for (var i = 0; i < state.segments.length; i++) {
      var s = state.segments[i];
      if (s && safeStr(s.soft_break_group) === gid) members.push(s);
    }
    members.sort(function (a, b) { return (a.soft_break_index || 0) - (b.soft_break_index || 0); });
    var starts = softBreakGroupMemberStarts(members);
    for (var m = 0; m < members.length; m++) {
      if (members[m] === seg || members[m].tid === seg.tid) { globalStart = starts[m]; break; }
    }
  }
  // Prefer the shared, unit-tested clip in EmphasisOverlay. If the module is
  // somehow unavailable (it is always loaded in the real app), inline the SAME
  // clamp+re-base — this helper must NEVER hand a consumer raw parent-global
  // offsets, or a soft-break tail would re-inherit a sibling's runs (the very
  // pollution this fix removes, reachable e.g. via the dev_translate bridge).
  // Mirrors EmphasisOverlay.clipRunsToLocalRange.
  if (typeof EmphasisOverlay !== "undefined" && EmphasisOverlay.clipRunsToLocalRange) {
    return EmphasisOverlay.clipRunsToLocalRange(runs, globalStart, globalStart + localLen);
  }
  var rangeEnd = globalStart + localLen;
  var clipped = [];
  for (var r2 = 0; r2 < runs.length; r2++) {
    var rr = runs[r2];
    if (!rr || typeof rr.start !== "number" || typeof rr.end !== "number") continue;
    var cs = Math.max(rr.start, globalStart);
    var ce = Math.min(rr.end, rangeEnd);
    if (ce <= cs) continue;
    var clone = {};
    for (var kk in rr) { if (Object.prototype.hasOwnProperty.call(rr, kk)) clone[kk] = rr[kk]; }
    clone.start = cs - globalStart;
    clone.end = ce - globalStart;
    clipped.push(clone);
  }
  return clipped;
}
...
// ── Emphasis-run overlay (target_emphasis_runs visibility) ──
//
// Marker-derived (faithful MT-carry) runs live in row.target_emphasis_runs
// (NOT annotations[]). This derives read-only overlay decorations so the
// translator SEES where the carried emphasis lands. There is no Accept/Dismiss
// affordance — format propagates automatically via faithful MT-carry; the
// translator edits content, not format (design-intent.md §5). The heuristic
// "Suggested emphasis" chip was removed 2026-06-24.
//
// Row-level state → per-run render mode:
//   target_emphasis_runs_auto === true            → "auto-carry" (dashed)
//   user_set === true && auto !== true            → "confirmed"  (solid)
//   otherwise                                     → not rendered (defensive)
// Only runs with a visible emphasis dimension are shown (font-split-only runs
// are import-side concerns, mirrored from renderEmphasisHtml).
function deriveEmphasisSuggestionDecos(row, text) {
  if (!row) return null;
  var runs = row.target_emphasis_runs;
  if (!Array.isArray(runs) || runs.length === 0) return null;
  var isAuto = row.target_emphasis_runs_auto === true;
  var isConfirmed = !isAuto && row.target_emphasis_runs_user_set === true;
  if (!isAuto && !isConfirmed) return null;
  var hasVisible = (typeof EmphasisOverlay !== "undefined" &&
    EmphasisOverlay._internal && EmphasisOverlay._internal.diffHasVisibleEmphasis)
    ? EmphasisOverlay._internal.diffHasVisibleEmphasis
    : null;
  var describe = (typeof EmphasisOverlay !== "undefined" && EmphasisOverlay.describeRun)
    ? EmphasisOverlay.describeRun : null;
  var len = safeStr(text).length;
  var out = [];
  for (var i = 0; i < runs.length; i++) {
    var r = runs[i];
    if (!r || typeof r.start !== "number" || typeof r.end !== "number") continue;
    if (hasVisible && !hasVisible(r.diff)) continue;
    var st = Math.max(0, Math.min(len, r.start));
    var en = Math.max(st, Math.min(len, r.end));
    if (en <= st) continue;
    out.push({ start: st, end: en, confirmed: isConfirmed, title: describe ? describe(r) : "" });
  }
 ...
 * `suggestions` (optional) = [{start,end,confirmed,title}] emphasis-run decos.
 */
function buildOverlayHtml(text, annotations, suggestions) {
  var s = safeStr(text);
  if (!s) return "";
  ...
  // Emphasis-suggestion decorations from target_emphasis_runs (read-only
  // overlay layer; the clickable Accept/Dismiss lives in a separate card chip
  // since the textarea sits on top of this overlay and swallows clicks).
  // `confirmed` runs (translator-accepted, non-_auto) render solid; auto
  // suggestions render dashed. Suppressed per-chunk where an annotation
  // already covers the range (avoids double-decoration with the markers-path
  // auto-annotation projection).
  var sugs = [];
  if (Array.isArray(suggestions)) {
    for (var sgi0 = 0; sgi0 < suggestions.length; sgi0++) {
      var sg0 = suggestions[sgi0];
      if (sg0 && typeof sg0.start === "number" && typeof sg0.end === "number" &&
          sg0.start >= 0 && sg0.end > sg0.start && sg0.end <= s.length) {
        sugs.push({ start: sg0.start, end: sg0.end, confirmed: !!sg0.confirmed, title: safeStr(sg0.title) });
      }
    }
  }
  ...
  // If nothing to decorate, plain text
  if (anns.length === 0 && tokens.length === 0 && sugs.length === 0) return escapeHtmlText(s);

  // If only tokens (no annotations, no suggestions), fall back to simple token rendering
  if (anns.length === 0 && sugs.length === 0) return tokenAwareHtml(s);
  ...
  for (var sgi = 0; sgi < sugs.length; sgi++) {
    pointSet[sugs[sgi].start] = true;
    pointSet[sugs[sgi].end] = true;
  }
  var points = Object.keys(pointSet).map(Number);
  points.sort(function (a, b) { return a - b; });
  ...
  for (var p = 0; p < points.length - 1; p++) {
    ...
    // Wrap with emphasis-suggestion span (innermost) — only where no
    // annotation covers this chunk, so the markers-path auto-annotation
    // highlight and the suggestion dashed underline never stack.
    if (activeAnns.length === 0 && sugs.length > 0) {
      for (var sgw = 0; sgw < sugs.length; sgw++) {
        if (sugs[sgw].start <= start && end <= sugs[sgw].end) {
          var sCls = sugs[sgw].confirmed ? "emp-confirmed" : "emp-suggest";
          chunk = '<span class="' + sCls + '"' +
            (sugs[sgw].title ? ' title="' + escapeHtmlAttr(sugs[sgw].title) + '"' : "") +
            '>' + chunk + '</span>';
          break;
        }
      }
...
function syncTokenOverlayForTextarea(area) {
  ...
  // Emphasis-run suggestion/confirmed decorations (target_emphasis_runs).
  // Skipped during IME composition (offsets are transiently out of sync; the
  // dashed underline reappears on compositionend's re-sync).
  var suggestions = null;
  if (tid && state.translationsByTid && state.translationsByTid[tid] && !area.__annComposing) {
    suggestions = deriveEmphasisSuggestionDecos(state.translationsByTid[tid], value);
  }

  // Cache key includes annotations + suggestions to detect changes
  var annKey = annotations ? JSON.stringify(annotations) : "";
  var sugKey = suggestions ? JSON.stringify(suggestions) : "";
  var cacheKey = value + "\x00" + annKey + "\x00" + sugKey;

  if (area.__tokenOverlayCacheKey !== cacheKey) {
    if (!value) {
      overlay.innerHTML = "";
    } else if (annotations || suggestions) {
      overlay.innerHTML = buildOverlayHtml(value, annotations, suggestions);
    } else {
